...
from .utils import is_null, is_scalar, is_integer
...
class Collection(CollectionBase, list):
    ...
    def _is_index(self, elem: Any) -> bool:
        ...
        if is_scalar(self.pool) and is_integer(self.pool):
            return True
        ...
        if len(pool) == 0 or not is_integer(pool):
            return True
        return False
    ...
    def _index_from_pool(self, elem: Any) -> Any:
        ...
        if self._is_index(elem):
            ...
            pool = (
                len(self.pool)
                if isinstance(self.pool, Sequence) or not is_scalar(self.pool)
                else self.pool
            )
...
class Slice(Collection):
    ...
    def _expand_pool(self, pool: int | Iterable) -> List:
