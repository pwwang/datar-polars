...
import polars as pl
from pipda import ContextBase, evaluate_expr

from .tibble import add_to_tibble
from .utils import vars_select, is_scalar
...
class Across:
    ...
    def __init__(
        ...
        cols = [data.columns[int(i)] for i in vars_select(data.columns, cols)]
    ...
    def evaluate(self, context: Enum | ContextBase = None):
        """Evaluate object with context"""
        if isinstance(context, Enum):  # pragma: no cover
...
class IfCross(Across, ABC):
    ...
    def evaluate(
        ...
        return (
            ...
            .select(
                pl.fold(
                    acc=pl.lit(self.__class__.ACC_INIT),
                    f=self.__class__.aggregate,
                    exprs=pl.col("*")
                )
            )
            .alias(f"__{self.__class__.__name__.lower()}")
        )
