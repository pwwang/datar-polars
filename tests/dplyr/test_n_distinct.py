...
def test_n_distinct_gives_correct_results():
    assert n_distinct([1, 2, 2, 3], __ast_fallback="normal") == 3
    assert n_distinct(pl.Series([1, 2, 2, 3]), __ast_fallback="normal") == 3


def test_n_distinct_treats_na_correctly():
    assert n_distinct(c(1.0, NA, NA), na_rm=False, __ast_fallback="normal") == 2
    assert n_distinct(pl.Series([1.0, None, None]), na_rm=True, __ast_fallback="normal") == 1


def test_n_distinct_scalar():
    assert n_distinct(4, __ast_fallback="normal") == 1
    assert n_distinct(NA, na_rm=True, __ast_fallback="normal") == 0
    assert n_distinct([1, 2, 3, 4], __ast_fallback="normal") == 4
